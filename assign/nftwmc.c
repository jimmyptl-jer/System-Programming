...
static int copy_or_move_files(const char* source_path, const struct stat* sb, int tflag, struct FTW* ftwbuf) {
    ...
    if (!excluded && tflag == FTW_F) {
        ...
        // Delete the file from the source directory if it's a move operation
        if (move_operation) {
...
int main(int argc, char* argv[]) {
    ...
    // Determine the operation (copy or move)
    if (strcmp(argv[0], "./cpmvdir_cp") == 0) {
        move_operation = 0;
    } else if (strcmp(argv[0], "./cpmvdir_mv") == 0) {
        move_operation = 1;
    } else {
